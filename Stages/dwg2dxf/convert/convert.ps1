<#
.SYNOPSIS
    把 convert/input_dwg/ 下的 .dwg 批量转成 .dxf 输出到 convert/output_dxf/。

.DESCRIPTION
    所有调用都在项目内：用项目自带的 uv 虚拟环境跑 dwg_converter，
    ODA 可执行文件由 dwg_converter 自动从 tools/oda/ 或 $PATH 探测。

    用法:
        .\convert.ps1                  # 转 input_dwg/ 下全部 .dwg
        .\convert.ps1 a.dwg            # 只转指定文件（相对 input_dwg/ 或绝对路径）
        .\convert.ps1 a.dwg b.dwg      # 转多个
        .\convert.ps1 -Clean           # 先清空 output_dxf/ 再转
        .\convert.ps1 -Verbose         # 详细日志
        .\convert.ps1 -Clean -Verbose a.dwg
...
$InputDir = Join-Path $ProjectRoot "convert" "input_dwg"
$OutputDir = Join-Path $ProjectRoot "convert" "output_dxf"
...
# ---- 拼装 uv 命令 ----
$UvArgs = @("run", "--project", $ProjectRoot, "python", "-m", "dwg_converter")
if ($Verbose) {
    $UvArgs += "-v"
}
