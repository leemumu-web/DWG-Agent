# convert —— DWG 批量转 DXF 工作区

放 DWG、出 DXF、跑脚本,一条命令完成批量转换。所有调用都在项目内
...
cp /path/to/*.dwg convert/input_dwg/

# 2. 跑脚本(项目根目录执行)
./convert/convert.sh

# 3. 拿结果
ls convert/output_dxf/    # 同名 .dxf 文件
```
...
```powershell
# 1. 把 dwg 文件丢进 input_dwg/
Copy-Item C:\path\to\*.dwg convert\input_dwg\

# 2. 跑脚本(项目根目录执行)
.\convert\convert.ps1

# 3. 拿结果
Get-ChildItem convert\output_dxf\    # 同名 .dxf 文件
```

`input_dwg/` 下的 **所有** `.dwg` 会被一次 ODA 调用批量转换,速度约
**4 个文件/秒**(实测 46 个真实图纸 11s)。产物文件名与源同名,仅后缀变 `.dxf`。

## 外部接口接入(最常用)

外部程序(后端服务、定时任务、CI)只需做三件事:

1. 把待转的 `.dwg` 复制/写入 `convert/input_dwg/`(或符号链接)。
2. 执行 `./convert/convert.sh --clean`(`--clean` 先清空旧输出,避免残留)。
3. 读 `convert/output_dxf/` 下的 `.dxf`,按同名映射回源文件。
...
# 写入待转文件
shutil.copy("user_upload.dwg", inp / "user_upload.dwg")
...
# 取产物
dxf = outp / "user_upload.dxf"
if dxf.exists():
    ...
```

退出码是判断成败的唯一可靠途径(脚本不输出机器可读格式)。失败时去
`convert/output_dxf/` 找 `<name>.dxf.err` 会被自动清理,所以失败信息只进 stdout/stderr,
...
./convert/convert.sh a.dwg           # 只转指定文件(相对 input_dwg/ 或绝对路径)
./convert/convert.sh a.dwg b.dwg     # 转多个
./convert/convert.sh --clean         # 先清空 output_dxf/ 再转
./convert/convert.sh -v              # 详细日志(DEBUG)
./convert/convert.sh --help          # 帮助
```

参数可组合:`./convert/convert.sh --clean -v a.dwg`。

**Windows PowerShell (convert.ps1):**

```powershell
.\convert\convert.ps1                     # 转 input_dwg/ 下全部 .dwg
.\convert\convert.ps1 a.dwg               # 只转指定文件
.\convert\convert.ps1 a.dwg b.dwg         # 转多个
.\convert\convert.ps1 -Clean              # 先清空 output_dxf/ 再转
.\convert\convert.ps1 -Verbose            # 详细日志(DEBUG)
.\convert\convert.ps1 -Clean -Verbose a.dwg
```
...
- 脚本用 `xvfb-run -a` 包裹 ODA(自动选空闲 display 号),可多实例并发跑不同 input 目录,
  但 **同一 `output_dxf/` 不要并发写**——会互相覆盖。多任务请各自用独立子目录或独立工作区。
...
# 批量
batch = convert("convert/input_dwg/", "convert/output_dxf/")
print(batch.ok, batch.total, batch.all_success)   # 46 46 True
