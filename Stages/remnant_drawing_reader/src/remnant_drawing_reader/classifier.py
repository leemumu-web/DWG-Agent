"""从图纸文本中提取候选信息（材质/项目号/零件号）的分类器。

候选提取由正则与领域常量驱动：零件号需同时含字母和数字且带连字符链
（如 SKG-D-4GZ-7）；``_MAX_PROJECT_LENGTH=128`` 防止把整段文字当项目号；
2-3 个汉字的短注释被排除；DATE/DWG/ISO/REV 等前缀被排除以免把日期/
文件名误判为候选。正则改动直接影响召回与误判，需回归样本验证。
"""
