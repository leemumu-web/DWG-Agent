...
本目录是独立 converter package，也是 `backend/` 的 editable dependency。平台只在 `DXF2DWG_PIPELINE_ENABLED=true` 时从 `dxf2dwg` Celery 队列调用。该包不负责用户认证、Job、存储授权或生产 FastAPI 服务。已跟踪 Linux AppImage 仍要求兼容主机、Xvfb/FUSE、ODA 许可审查和真实代表性 DXF 验证。
