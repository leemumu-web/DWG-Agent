...
用法:
    python -m dxf_converter --check-env
    python -m dxf_converter path/to/a.dxf [-o out]
    python -m dxf_converter path/to/dir  [-o out] [-r]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="dxf_converter",
        description="DXF → DWG 转换（基于 ODA File Converter）",
    )
    parser.add_argument(
        "source", nargs="?",
        help="单个 .dxf 文件或包含 .dxf 的目录（--check-env 时可省略）",
    )
    parser.add_argument(
    ...
    parser.add_argument("--version", default="ACAD2018", help="DWG 版本 (默认 ACAD2018)")
