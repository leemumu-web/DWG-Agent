"""ODA File Converter 引擎：用 subprocess 调用外部 ODA CLI 完成 DXF→DWG。
...
ODA 失败时的静默行为：returncode 仍是 0，只在目标目录写 <name>.dwg.err 副产物。
...
class OdaConverter:
    ...
    def _collect_result(
        ...
        ODA 成功时写 <stem>.dwg；失败时 returncode 仍是 0 但写 <stem>.dwg.err。
