"""框架集成适配层：将 dxf_converter 的错误/结果映射为 FastAPI 兼容格式。
...
ERROR_CODES = {
    ...
    "DWG_SOURCE_MISSING": "DWG_SOURCE_MISSING",
    "DWG_TIMEOUT": "DWG_TIMEOUT",
}
...
def _failure_code(result: ConvertResult) -> str:
    ...
    if "timeout" in err or "超时" in err:
        return ERROR_CODES["DWG_TIMEOUT"]
    if "not found" in err or "不存在" in err:
        return ERROR_CODES["DWG_SOURCE_MISSING"]
    return ERROR_CODES["DWG_CONVERSION_FAILED"]
