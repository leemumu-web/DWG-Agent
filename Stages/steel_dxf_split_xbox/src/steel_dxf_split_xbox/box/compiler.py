...
def compile_box_core(
    ...
    *,
    ...
    metadata = run_analysis(source, family=family)
...
def compile_box(
    ...
    *,
    ...
    if config.family not in {"BOX", "XBOX"}:
        raise ValueError(f"unsupported closed-box family: {config.family}")
    if config.family == "BOX":
        ...
        release_attestation = load_verified_xbox_release_attestation(
            config.release_attestation_path
        )
