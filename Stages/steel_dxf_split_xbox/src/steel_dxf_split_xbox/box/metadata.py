...
class BoxProfile:
    ...
    def canonical(self) -> str:
        ...
        if self.extra is not None:
            values = (*values, self.extra)
        return self.family + "*".join(_format_dimension(value) for value in values)
...
_XBOX_BOX5_RE = re.compile(
    rf"\bBOX\s*({_NUMBER})\s*[*X×]\s*({_NUMBER})"
    rf"\s*[*X×]\s*({_NUMBER})\s*[*X×]\s*({_NUMBER})"
    rf"\s*[*X×]\s*({_NUMBER})\b",
    re.IGNORECASE,
)
_XBOX_HK_RE = re.compile(
    rf"\bHK\s*({_NUMBER})\s*-\s*({_NUMBER})\s*-\s*({_NUMBER})"
    rf"\s*[*X×]\s*({_NUMBER})\s*-\s*({_NUMBER})\b",
    re.IGNORECASE,
)
...
def parse_xbox_profile(value: str) -> BoxProfile:
    """Parse either approved five-field XBOX business dialect."""

    normalized = normalize_text(value)
    match = _XBOX_BOX5_RE.fullmatch(normalized)
    dialect = "BOX5"
    if match is None:
        match = _XBOX_HK_RE.fullmatch(normalized)
        dialect = "HK"
    if match is None:
        raise MetadataResolutionError(f"not an XBOX BOX5/HK profile: {value!r}")
    dimensions = tuple(float(part) for part in match.groups())
    if dialect == "BOX5":
        height, width, web_thickness, flange_thickness, extra = dimensions
    else:
        height, web_thickness, flange_thickness, width, extra = dimensions
    if min(height, width, web_thickness, flange_thickness, extra) <= 0:
        raise MetadataResolutionError("XBOX dimensions and extra must be positive")
    if 2.0 * flange_thickness >= height:
        raise MetadataResolutionError(
            "XBOX flange thickness leaves no positive web clear width"
        )
    if 2.0 * web_thickness >= width:
        raise MetadataResolutionError(
            "XBOX web thickness leaves no positive flange clear width"
        )
    return BoxProfile(
        height=height,
        width=width,
...
def _resolve_metadata(source: SourceDocumentIR, *, family: str) -> BoxMetadata:
    """Resolve one complete closed-box title record with source evidence."""
    ...
    for entity in text_entities:
        ...
        if family == "BOX" and not normalized.upper().startswith("BOX"):
            continue
        if family == "XBOX" and not normalized.upper().startswith(("BOX", "HK")):
            continue
        try:
            parser = parse_box_profile if family == "BOX" else parse_xbox_profile
            profile_candidates.append((entity, parser(normalized)))
        except MetadataResolutionError:
            continue
    if not profile_candidates:
        raise MetadataResolutionError(f"no valid {family} profile found")

    canonical_profiles = {profile.canonical for _, profile in profile_candidates}
    if len(canonical_profiles) != 1:
        raise MetadataResolutionError(
            f"conflicting {family} profiles: " + ", ".join(sorted(canonical_profiles))
        )
    title_group_ids = {entity.group_id for entity, _ in profile_candidates}
    if len(title_group_ids) != 1:
        raise MetadataResolutionError(
            f"equivalent {family} profiles occur in multiple title groups"
        )
...
def resolve_box_metadata(source: SourceDocumentIR) -> BoxMetadata:
    return _resolve_metadata(source, family="BOX")


def resolve_xbox_metadata(source: SourceDocumentIR) -> BoxMetadata:
    return _resolve_metadata(source, family="XBOX")
