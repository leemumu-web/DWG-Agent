...
def _structured_plate_type(roles: tuple[str, ...], *, family: str) -> str:
    role_kinds = {
        "腹" if role.startswith("web_") else "翼" if role.startswith("flange_") else ""
        for role in roles
    }
    if len(role_kinds) != 1 or "" in role_kinds:
        raise ValueError(f"{family} output group mixes unsupported plate roles: {roles}")
    return family + next(iter(role_kinds))


def _layout_report(layout: BoxLayout, *, family: str) -> list[dict[str, object]]:
    return [
        {
            "group_id": plate.group_id,
            "roles": [role.value for role in plate.roles],
            "part_type": _structured_plate_type(
                tuple(role.value for role in plate.roles),
                family=family,
            ),
    ...
    *,
    ...
    with TemporaryDirectory(prefix=".box-v1-stage-", dir=output_root) as temporary:
        ...
        report: dict[str, object] = {
            ...
            "report_schema": (
                BOX_COMPILATION_REPORT_SCHEMA
                if config.family == "BOX"
                else "XBOX-COMPILATION-REPORT-1.0"
            ),
            "profile_family": config.family,
            "core": {
                "version": BOX_CORE_VERSION,
                "tag": BOX_CORE_TAG,
            ...
            "metadata": {
                ...
                "profile_source_dialect": core.metadata.profile.value.source_dialect,
                "profile_extra": core.metadata.profile.value.extra,
            },
            ...
            "weld_allowance_output_groups": _layout_report(
                layout,
