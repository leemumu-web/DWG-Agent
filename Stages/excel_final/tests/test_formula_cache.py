...
def test_ooxml_formula_patch_can_preserve_an_intentionally_blank_cache(tmp_path) -> None:
    cache = importlib.import_module("ooxml_formula")
    path = tmp_path / "manual-formula.xlsx"
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "整理表"
    sheet["A1"] = 1000
    ...
    cache.patch_formula_caches(
        path,
        "整理表",
        {"D1": cache.FormulaCache(formula, None)},
    )

    formulas = load_workbook(path, data_only=False, read_only=True)
    values = load_workbook(path, data_only=True, read_only=True)
    try:
        assert formulas["整理表"]["D1"].value == formula
        assert values["整理表"]["D1"].value is None
    finally:
        formulas.close()
        values.close()
