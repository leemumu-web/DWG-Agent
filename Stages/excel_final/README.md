...
D 系列按材质路由：HRB 查螺纹钢，HPB/Q235B/Q355B 查圆钢。`HN/HW/HM` 和 `Cxx` 只在 MySQL 查询边界转换为数据库已有的 `Hxx` 和 `[xx` 键，Excel 仍显示来源规格。标准型材理论毛重超过 2% 仍为严重；板材及 BH/BOX/BT 几何偏差只提示复核，源单重×数量链和净重大于毛重继续作为硬隔离条件。
