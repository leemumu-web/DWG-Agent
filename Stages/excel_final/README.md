...
`tools/compare_supervised_sample.py`用于把规范生产结果与经整理的两表人工监督样本进行诊断性对照。它只比较双方共同非空的单元格：GT空值不是程序必须补齐的要求，程序独有的公式结果也不因GT缺值而判错。`整理表`先按构件和导入零件身份匹配，再用序号、规格、宽度、长度、下料长度、材质和截面联合消歧；`part`优先按构件范围、导入零件号、规格、宽度、下料长度和材质完整键匹配。当一侧构件编号为空时，允许按双方共有的导入零件身份降级匹配并继续消歧；两侧非空构件编号冲突时禁止跨构件匹配，同分候选明确标记为歧义。
