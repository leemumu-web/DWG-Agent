...
## 2026-07-31 Excel 第二阶段 BH 左右进接入验收

本轮把 Excel 第二阶段接入正式生产工作流：只读取当前项目、当前分类 attempt 冻结的拆板前
BH DXF 和当前 Excel 第一阶段正式结果。后台先生成 BH 左右进读取表，再重建并校验第一阶段
基线，按零件号深化整理表与 part 表；两份 Excel 分别下载。提交后页面停留在第二阶段，
显示真实排队/处理进度，完成后只解锁人工屏障，不自动切换操作页面。

| 门禁 | 结果 | 当前证据 |
|---|---|---|
| Backend 全量 | **1722 passed，10 skipped** | 架构、分类、拆板、Excel、工作流、权限、安全、基础设施与回归套件零失败；23 条为依赖弃用或测试短密钥 warning。 |
| 第二阶段聚焦 | **286 passed** | 覆盖 BH 读取契约、多翼板增行、数量和重量同步、part 同步、公式缓存、无 BH 原样输出、跨项目/跨 attempt 拒绝、两份产物登记与下载。 |
| Quick 门禁 | **PASS=6 FAIL=0 BLOCKED=0** | Shell、Ruff、211 个 HTTP operation、16 个 Celery task、146 个分区文档、272 项聚焦回归、API 文档和生产构建通过。 |
| 工作流浏览器 | **10 passed** | 覆盖预检、当前项目执行请求、启动后即时状态、运行中防重复、工人中文错误、阶段停留、移动端布局、BH 读取表和第二阶段结果分别下载。 |
| 前端生产构建 | **pass** | 129 个源码文件、12 个 feature 边界、TypeScript 与 Vite production build 通过；仅保留既有公共依赖大 chunk 提示。 |
| 运行边界 | **未改生产数据** | 本轮在本地测试数据库、模拟对象存储与浏览器路由上验收，没有启动 18080 的生产功能开关，也没有修改工作流 5/6 或远端 gg 数据。 |

## 2026-07-26 生产工作台、Excel 第一阶段与加密服务器发布验收

本轮把工人工作台收敛为真实生产项目入口，并补充六段可展开操作手册；所有页面错误统一为
中文原因、建议动作和请求编号，不显示后端错误码、Traceback、数据库驱动、容器名或本机路径。
数据控制台只管理当前生产任务与已登记文件。服务器模板只启用主流程所需的 DWG→DXF、分类、
拆板和 Excel 第一阶段，DXF→DWG 与通用 DXF→Excel 继续关闭。

| 门禁 | 结果 | 当前证据 |
|---|---|---|
| 真实生产流程 | **pass** | 临时项目上传真实 BH DWG 与真实 Tekla XLSX；DWG Job `5` 生成规范 DXF，分类 Job `6` 以 `TITLE_PROFILE_PROVED` 确认为 `BH250*200*8*14`，拆板 Job `7` 形成原长/余量两份 DXF并通过独立复开，Excel Job `8` 完成并登记唯一 `stage1_excel`。另一张仅文件名含 BH、标题字段缺失的整理图正确进入人工确认，没有误投拆板。 |
...
## 2026-07-26 批量传输进度与容器存储稳定性收口

当前所有可达的大批量图纸入口统一按实际字节显示进度：文件夹/多文件/ZIP 上传、工作流输入、
分类与拆板 ZIP、阶段/全流程归档、完整备份、DXF 批次下载和余料批量导入均复用同一传输模型。
未知响应总量显示已传输字节；有响应长度时显示百分比；文件名以服务端
`Content-Disposition` 为准。单个 Excel 和单张图纸下载不伪装为批量进度。
...
| 备份探针 | **pass** | `dwg_agent`、`hardware_handbook` 与 MinIO volume 完整归档；SHA-256、gzip 和 tar 均可读。极简 MinIO 镜像不含 tar，脚本改由临时后端工具容器继承同一 volume；不新增常驻服务。 |

## 2026-07-25 生产工作流、拆板交付与转换结果生命周期收口

本轮吸收远端 BH 余量链实现，并在统一主线完成拆板、工作流前端、Excel 第一阶段和独立转换
页面的收口。工作流完成当前阶段后只解锁下一阶段，不自动切换操作页面；拆板正式包只提供
`原长`、`余量增长后短文件` 两个目录，另提供本批全部分类原图下载。Excel 第一阶段执行前
使用同一冻结输入规则预检，正式结果下载只返回一份 XLSX。独立 DWG/DXF 转换现在区分
“任务成功”和“结果文件仍可用”；历史结果已释放时不再显示会返回 404 的下载按钮，而是
明确提示并以源文件创建新任务。

| 门禁 | 结果 | 当前证据 |
|---|---|---|
| 真实 40 图拆板 | **40 / 40 自动接纳** | 从生产工作流 18 的数据库分类结果读取 40 个源 DXF；20 个 BH、20 个 BOX 全部形成原长与余量增长配对，0 人工、0 失败。两个此前缺失目标均已形成配对；完整执行约 250.8 秒。 |
| BOX 发布门 | **20 / 20 passed** | 10 个校准样本与 10 个验收样本全部通过，输入/参考基线未改变；运行时 attestation 已按当前实现指纹刷新并有回归锁定。 |
| Backend 全量 | **1496 passed，10 skipped** | Job/文件生命周期、Excel 单文件结果、工作流预检和解锁、拆板分流/下载以及全部既有后端回归无失败。 |
| Frontend 全量 | **116 passed，7 skipped** | 共 123 个 Playwright 场景，0 失败；第三阶段完成后停留当前页、第四阶段解锁、原生拆板下载、Excel 冻结输入和结果释放重新提交均通过。跳过项由实时数据或真实 XLS 开关条件决定。 |
| 前端生产构建 | **pass** | 128 个 TypeScript 源文件、12 个 feature 边界、TypeScript 与 Vite production build 通过；仅保留既有大 chunk 提示。 |
| 架构与基础设施 | **74 / 74；122 / 122** | 169 个 path、197 个 operation 的运行时合同，模块/分区文档、Nginx、16 个 Compose service、MySQL 应用路径与 root 授权审计、环境键、文件完整性和死代码门禁通过。 |
| 统一 full 门禁 | **PASS=18 FAIL=0 BLOCKED=0** | Shell、Ruff、架构、文档、239 条聚焦回归、全量后端、Alembic、基础设施、Compose、空 MySQL 迁移、五组 Stage、拆板运行时、生产构建和全量浏览器在同一最终工作树通过。 |
| 活动生产栈 | **pass** | MySQL 55 张表，9 类 Celery worker、FastAPI `:8010`、Nginx/SPA `:8080` 全部健康且运行源码为最新。 |
| 活动 MySQL 性能 | **pass** | 200 条独立 DWG 文件列表连续采样约 46–57ms；194 条最新转换任务连同结果可用性约 28–33ms。180 个成功任务中识别 178 个可下载、2 个结果已释放。 |
...
jobs 现按创建、attempt 生命周期、Celery 投递、事件流、恢复、复核和 HTTP 用例分层，领域模块独占 `jobs`、`job_dispatches`、`job_steps`、`analysis_results`、`review_records` 五张表。13 个 Job、4 个 Result、1 个 Review operation 的 method/path/function-name 保持不变，静态 Job 路径先于 `/{job_id}` 组合；其他业务域只通过 `app.modules.jobs.interface` 调用。平台消息层只保留通用 Celery app、SQL transport 与 worker-ready callback，Job stale 恢复由 bootstrap 注册，platform 不再反向导入业务模型。Job 与投递意图已同事务提交，独立 dispatcher 以短 lease、稳定 task ID 和有界退避完成消息发布；SSE 仍读取 `jobs.progress_data` 最新快照，没有持久事件编号和 replay。
