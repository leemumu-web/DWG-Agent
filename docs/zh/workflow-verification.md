...
执行 `stop-all.sh` 停止全部应用层服务。MySQL（MariaDB, systemd）和 Redis（Valkey 9.x, systemd）作为共享基础设施保留运行。
...
通过 `db.sh reset` 删除并重建 `dwg_agent` 数据库，执行全部 4 次 Alembic 迁移，写入种子数据（7 角色 + 8 权限 + 1 超级管理员）。
...
<base> → 40452ddd24e7 (initial — 创建全部 17 张业务表)
       ...
       → 53cd59adf848 (add_batch_name_to_files — files.batch_name VARCHAR(128) + 索引) [head]
```
...
| **Redis (Valkey 9.x)** | `systemd` 自启动 | `redis-cli ping` → `PONG` |
...
1. **扩展名白名单** — `ALLOWED_UPLOAD_EXTENSIONS = {".dwg", ".dxf", ".zip"}`；其余一律 415 `FILE_TYPE_NOT_ALLOWED`。
2. **MIME 类型检查（仅提示）** — `validate_upload_mime` 识别一组 DWG MIME 类型及通用二进制回退类型，但从不拦截；DWG 文件头才是真正的安全边界。
3. **DWG 文件头验证** — 对 `.dwg` 文件，首 6 字节必须匹配 `AC1012`-`AC1032`（覆盖 AutoCAD R13 至 2018+），否则 415 `FILE_NOT_DWG`。
4. **大小强制** — `.dwg` 最小 1,024 bytes（`MIN_DWG_SIZE_BYTES`）；所有上传最大 `MAX_UPLOAD_SIZE_MB`（默认 512 MiB），否则 413 `FILE_TOO_LARGE`。
5. **流式哈希** — SHA-256 + MD5 在按 1 MB 分块读取时同步计算。
...
由于 `extract_layers` 不属于三种真实转换任务类型（`convert_dwg_to_dxf` / `convert_dxf_to_dwg` / `extract_dxf_to_excel`），`job_service` 会将其映射到 `local_stub` 管线并路由到 `report` 队列——这正是 Stage 1 预期的骨架链路检查。

#### 2.3.5 Celery 自动执行

任务投递到 Redis broker 的 `report` 队列后，Celery worker-report 节点（`report-local@archlinux`）自动拉取执行。Stage 1 使用假任务体 `run_local_stub_job`，模拟完整的 queued→running→succeeded 生命周期，验证 Celery 调度、状态变迁、job_steps 写入、analysis_results 写入全链路。
...
GET /api/v1/jobs/1/results
  result_type: extract_layers   ← 等于 job.task_type
...
下载端点额外要求认证（URL 非独立 capability token，defense-in-depth），下载前校验上传者/管理员/项目成员身份。`compare_digest()` 常量时间比较防止签名时序攻击。下载响应为原始 `FileResponse`/`StreamingResponse`（不包裹信封）。
...
→ 200 OK, 1 条审核记录（信封内纯列表，不分页）
```

`decision` 有效值：`approved`、`rejected`、`needs_revision`。
...
```
  ...
  ──────────────────────────────  ────────
  18 tables total (17 张业务表 + alembic_version)
```
...
工程师 zhangwei 登录（engineer）
  ...
  │     └─ 5 层安全校验全部通过
  │       ① 扩展名 .dwg → ② MIME（仅提示） → ③ header AC1027 → ④ ≥1024B ≤512MB → ⑤ SHA256+MD5
  │
...
| API 模块覆盖 | 12/12（Auth / Users / Roles / Projects / Files / Drawings / Jobs / Results / Reviews / Audit / Agent / System） | ✅ |
| 业务场景步骤 | 24 个检查点 | ✅ |
| 审计记录 | 26 条（18 种操作类型） | ✅ |
| 数据库表 | 18 张（29 FK, ~45 索引） | ✅ |

**结论：DWG-Agent Stage 1 平台骨架全链路功能正常。所有基础设施组件、74 个 API 端点（73 个位于 `/api/v1` 下 + 根 `GET /health`）、RBAC 权限模型、JWT 认证体系（含 jti 黑名单和时序攻击防御）、DWG 文件上传（5 层安全校验）、Celery 异步任务调度、HMAC 签名下载、审计日志不可变追踪、软删除与级联归档、状态守卫机制均通过验证。**
