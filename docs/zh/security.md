...
Secure cookie 默认值跟随 `APP_ENV=production`。公网部署需要真实 TLS 和 Secure cookie。设置 `REFRESH_COOKIE_SECURE=false` 只是私有 HTTP 网络的显式风险接受；当前 Compose 尽管映射宿主 443，也不终止 TLS。
...
Nginx 当前没有 TLS server、证书处理或 HSTS。rate limit 和 header 可以降低暴露面，但不能让不可信网络上的明文登录安全。
