...
> 英文对应文档：[../operations.md](../operations.md)
...
| Nginx 80 可达 | HTTP 网关和静态根响应 | 443 TLS 已配置 |
...
`migration-test` 创建并删除临时 schema。它证明从空库升级到 `a74c2e9f1d30`，不测试 downgrade 或生产数据迁移时长。
...
仓库有本地 MySQL backup/restore helper，但没有 Compose 全局协调备份服务。可恢复集合必须包括：
...
静止 Compose 数据库 dump 前，显式停止所有 writer；Compose service name 不支持 `worker-*` wildcard：
...
该数据库 dump 单独不够。重启 writer 前，应通过 internal network 上已测试的 `mc mirror`/snapshot 路径采集 MinIO 数据。仓库没有提供该 one-off client 或数据库/对象原子快照编排。
...
随后恢复全部对象 bucket，运行 `alembic current`，把代表性 `files.sha256` 与字节比较，启动服务并执行完整工作流。恢复 broker row 可能重新引入 queued delivery；启动 worker 前检查 queued/running Job 和 broker table。
