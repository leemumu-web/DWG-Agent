...
公开 `/workflows` 路由已经接通文件引用、DXF→Excel Job、Excel Final Job、Job attempt 同步、结果产物挂接和 active Job 取消。拆板算法、CAM 工作包算法、Windows Node Agent 与 SinoCAM 尚无服务器实现；它们拥有稳定阶段、输入输出和执行端点，但调用返回真实 501 边界，不伪造成功。
...
|---:|---|---|---|
| 1 | `source_intake` | manual | 复用 `/files` 绑定源文件；至少一个 file artifact 后才能冻结输入 |
...
- `source_intake` 无源文件返回 409 `WORKFLOW_SOURCE_FILE_REQUIRED`；
