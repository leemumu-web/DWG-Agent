...
`migration-test` 创建并删除临时 schema，并顺带清理历史崩溃残留的临时库；当前目标为 `b4e8c2a7d910` 和 45 张模型表，额外验证生产输入、DXF 分类、DXF 拆板及复核决定、余料库存、控制平面与每日归档账本、`jobs.request_key`/唯一约束及种子数据兼容；它不测试 downgrade 或生产数据迁移时长。需 `sudo mariadb` 的子命令先经 `ensure_sudo` 预检，无 TTY 且凭据未缓存时快速失败而非挂起。
