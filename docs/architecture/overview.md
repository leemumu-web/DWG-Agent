...
local 后端 `put_fileobj` 经临时文件、`fsync` 和原子 `os.replace` 落盘。MinIO/local 都实现 stat、exists 和游标分页清单。`operations/storage_reconciliation` 的 report-queue task 异步生成 files 域拥有的 `storage_scan_runs` 与异常 `storage_scan_findings`，分类为对象缺失、未登记对象、大小不符和软删除对象保留。后台保留带签名预检 token、5 分钟有效期、实时摘要重检、批量数量/字节上限和幂等键的处置接口，但生产人员数据管理台不暴露这些高风险入口。`reap-storage` 仍用于保留期回收和脚本化维护，不替代扫描/处置账本。

数据管理台现有“生产任务”和“文件存储”两个页签，均读取既有业务接口。每日归档与一致性处置保留为后台维护能力：归档按 `Asia/Shanghai` 自然日生成带签名的冻结清单，maintenance worker 流式读取源对象，在 `dwg-reports/daily-archives/YYYY-MM-DD/` 新增 ZIP 和 JSON manifest，并把两者登记到 `files`/`file_transfers`；不移动或删除源对象。历史归档前缀自动排除，避免递归打包。相同清单复用成功结果，活动任务复用运行记录；源文件状态、摘要或对象可读性变化时整次失败，不返回貌似完整的 ZIP。完整页面边界见[运维指南](../guides/operations.md#数据控制台运行手册)。
...
SQL transport 没有 fanout remote control。worker 健康使用 Celery PID 加 `worker_ready` marker，不使用 `inspect`。控制平面额外将 Celery lifecycle/task signal 最佳努力写入 `worker_runtimes` 和 `control_plane_events`；其 `last_seen_at` 是活动观测而非 broker lease。数据管理台不展示内部 broker 行数、Worker 或消息表；这些仅供后台诊断。result row 在 24 小时后过期；业务 Job 历史没有自动保留策略。
