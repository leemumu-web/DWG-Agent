...
| `MYSQL` | platform + 所有业务模块 | implemented | MySQL 是业务事实源；迁移管理 46 张模型表 |
...
| SQLAlchemy metadata/session/mixin | `app/platform/database/` | `bootstrap/model_registry.py` 显式加载模型 owner 和 46 张表；files、jobs、workflows、dxf_splitting 与 remnant_inventory 分别通过领域模型包装配其多张表。 |
