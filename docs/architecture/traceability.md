...
| `/workflows` | `app/modules/workflows/` | 拥有 run/stage/artifact/input/export/retention 七张表；模板、状态机、Job 同步、阶段执行、输入转换、拆板投影/下载、完整备份和 81 个聚合 HTTP operation 分层；其他模块只经 `workflows.interface`。 |
