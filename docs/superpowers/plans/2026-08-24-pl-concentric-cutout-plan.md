# PL Concentric Cutout Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove only the smaller of proven concentric circular PL cutouts while preserving the outer circle and every unrelated hole or slot.

**Architecture:** Add one PL-private filter in `geometry.py` after all contained and Bolt cutout groups have been collected. The filter proves circular groups from native ARC center/radius evidence, preserves source order, and removes a group only when a strictly larger circular group shares its center within the existing `0.1 mm` topology tolerance.
...
- Treat centers within `0.1 mm` as the same center.
- Remove an inner circle only when the outer radius exceeds it by at least `0.1 mm`.
- Keep different-center circles, intersecting circles, ellipses, slots, and unproved shapes unchanged.
...
### Task 1: Filter proven concentric circular cutout groups
...
- Produces: `_without_concentric_inner_circles(groups) -> tuple[tuple[DXFEntity, ...], ...]`, preserving group order and all non-matching groups.

- [ ] **Step 1: Add anonymous positive and negative fixtures**

Extend `_save_geometry_source()` with `concentric_offset_mm: float | None = None`. When supplied, create a Part-layer radius-10 circle from two native arcs at `(200 + offset, 110)` and a Bolt-layer radius-20 circle at `(200, 110)`:

```python
if concentric_offset_mm is not None:
    center = (200.0 + concentric_offset_mm, 110.0)
...
```python
def test_concentric_cutouts_keep_only_the_outer_circle(tmp_path: Path) -> None:
    geometry = importlib.import_module("steel_dxf_split.pl.geometry")
    drawing = tmp_path / "concentric-hole.dxf"
    _save_geometry_source(drawing, concentric_offset_mm=0.0)
...
def test_different_center_cutouts_are_both_preserved(tmp_path: Path) -> None:
    geometry = importlib.import_module("steel_dxf_split.pl.geometry")
    drawing = tmp_path / "offset-holes.dxf"
    _save_geometry_source(drawing, concentric_offset_mm=0.2)
    context, metadata = _load_geometry_context(drawing)
...
```powershell
$env:PYTHONPATH='C:\Users\李某\AppData\Local\Temp\codex-pl-splitter-py312\Lib\site-packages;E:\桌面\DWG-Agent\.worktrees\codex-pl-professional-compare\Stages\steel_dxf_split_v1.5.2\src'
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m pytest backend/tests/dxf_splitting/test_pl_splitter.py -k 'concentric_cutouts or different_center_cutouts' -q
```

Expected: the concentric positive test fails with two cutout groups; the `0.2 mm` offset negative test passes.
...
def _without_concentric_inner_circles(
    ...
    return tuple(
        ...
        or not any(
            ...
            and dist(signature[0], other[0]) <= TOPOLOGY_TOLERANCE_MM
...
```python
cutout_groups = list(
    _without_concentric_inner_circles(tuple(cutout_groups))
)
```
...
```powershell
$env:PYTHONPATH='C:\Users\李某\AppData\Local\Temp\codex-pl-splitter-py312\Lib\site-packages;E:\桌面\DWG-Agent\.worktrees\codex-pl-professional-compare\Stages\steel_dxf_split_v1.5.2\src'
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m pytest backend/tests/dxf_splitting/test_pl_splitter.py -k 'cutout or hole' -q
```
...
```python
if item["part_number"] in CONCENTRIC_OUTER_ONLY_PARTS:
...
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m pytest backend/tests/dxf_splitting/test_pl_paired_corpus.py -q
```
...
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m pytest backend/tests/dxf_splitting/test_pl_real_corpus.py -q
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m pytest backend/tests/dxf_splitting -q
```
...
```powershell
$env:PYTHONPATH='C:\Users\李某\AppData\Local\Temp\codex-pl-splitter-py312\Lib\site-packages;E:\桌面\DWG-Agent\.worktrees\codex-pl-professional-compare\Stages\steel_dxf_split_v1.5.2\src'
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m ruff check Stages/steel_dxf_split_v1.5.2/src/steel_dxf_split/pl/geometry.py backend/tests/dxf_splitting/test_pl_splitter.py backend/tests/dxf_splitting/test_pl_paired_corpus.py
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m ruff format --check Stages/steel_dxf_split_v1.5.2/src/steel_dxf_split/pl/geometry.py backend/tests/dxf_splitting/test_pl_splitter.py backend/tests/dxf_splitting/test_pl_paired_corpus.py
...
Verify `E:\桌面\PL板材\122个PL原图_同心孔修正版_20260824` does not exist, then run the PL-only CLI without `--overwrite`:

```powershell
Test-Path -LiteralPath 'E:\桌面\PL板材\122个PL原图_同心孔修正版_20260824'
$env:PYTHONPATH='C:\Users\李某\AppData\Local\Temp\codex-pl-splitter-py312\Lib\site-packages;E:\桌面\DWG-Agent\.worktrees\codex-pl-professional-compare\Stages\steel_dxf_split_v1.5.2\src'
& 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -m steel_dxf_split.pl.cli 'E:\桌面\DWG-Agent\.tmp\pl_pairs_20260822\dxf_source' --output-dir 'E:\桌面\PL板材\122个PL原图_同心孔修正版_20260824'
```
...
output_dir = Path(r"E:/桌面/PL板材/122个PL原图_同心孔修正版_20260824")
...
'@ | & 'C:\Users\李某\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' -
```
...
git commit -m "fix: omit inner concentric PL cutouts"
```
