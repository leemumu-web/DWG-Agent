...
**Goal:** 让 Excel Final 的五张生成表按实际内容自适应列宽，同时保持原表和业务数据不变，并在最终 Excel 删除不展示的内部审计列。

**Architecture:** 在 `writer_parts.py` 的最终格式化阶段先保存可选的完整内部导入副本，再按表头删除不展示的内部列并集中计算文本显示宽度。普通列使用 8–32 的边界，处理报告长文本列使用 16–48 并换行；后端从任务临时副本入库，只持久化最终删列文件。
