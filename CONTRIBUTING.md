...
本项目当前是内部 v0.1 技术预览版。先阅读[技术预览指南](docs/developer-preview.md)、[企业平台技术规范](DWG-Agent企业平台技术规范.md)和受影响子系统文档。不要通过启用占位 Agent/CAD worker、增加内存 fallback 或放宽授权来让测试通过。
...
2. 运行 `make docs-generate` 更新生成的 `docs/api.md`。
...
Stage 源码、锁文件和最小测试可以跟踪；真实 corpus、ODA 许可资产、生成工作簿、cache 和 virtualenv 不得因测试方便进入提交。平台调用契约写入 `docs/processing-pipelines.md`，算法细节保留在 Stage README。
...
只维护根入口、组件 README 和 `docs/*.md` 中文文档。`docs/api.md` 是生成文件，不手改。能力声明须分别说明：代码是否存在、默认 flag、外部依赖、验证环境/日期和剩余限制。
