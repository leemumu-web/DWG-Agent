...
本 feature 是网页端余料库唯一 owner。`RemnantInventoryPage.tsx` 组合页面和 URL 查询状态；`RemnantSearchPanel.tsx` 管理材质、厚度、系列与状态条件；`RemnantDetailDrawer.tsx` 展示占用信息和库存动作；`RemnantEditModal.tsx` 供管理员修订正式库存；`RemnantMaterialCatalog.tsx` 管理标准材质、系列、后缀和别名；`api.ts`、`types.ts` 定义后端契约；`styles.css` 保持现有管理端视觉边界；`index.ts` 是跨 feature 唯一入口。
