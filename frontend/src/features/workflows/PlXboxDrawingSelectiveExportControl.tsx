...
import {
  ...
  getPlXboxSelectiveExportPreview,
  downloadPlXboxSelectiveExport,
} from './workflows.api';

export function PlXboxDrawingSelectiveExportControl({
  workflowId,
  runId,
  disabled = false,
  disabledReason,
  ...
  const [selected, setSelected] = useState<PlXboxSelectiveExportCategory[]>([]);
  const [prepared, setPrepared] = useState<PlXboxSelectiveExport | null>(null);
  const [downloadProgress, setDownloadProgress] = useState<TransferProgress | null>(null);

  const previewQ = useQuery({
    queryKey: ['pl-xbox-selective-export-preview', workflowId, runId],
    queryFn: () => getPlXboxSelectiveExportPreview(workflowId, runId!),
  ...
  const downloadM = useMutation({
    mutationFn: (next: PlXboxSelectiveExport) => {
      const handle = downloadCtrl.start();
      return downloadPlXboxSelectiveExport(next, setDownloadProgress, handle.signal)
  ...
  const createM = useMutation({
    mutationFn: () => {
      ...
      return createPlXboxSelectiveExport(workflowId, runId, selected);
    },
    onSuccess: (next) => {
      setPrepared(next);
      setDownloadProgress(null);
          ...
          {previewQ.data && !previewQ.isFetching && (
            <Checkbox.Group
              ...
              onChange={(values) => setSelected(
                values as PlXboxSelectiveExportCategory[],
              )}
            >
