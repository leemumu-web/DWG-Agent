...
export function DrawingProcessingPanel({
  ...
  const allDrawingsArchiveM = useMutation({
    ...
    onError: (error) => message.error(
      describeApiError(error, '本批原图压缩包下载失败'),
    ),
  });
          ...
          {run.status === 'completed' ? (
            ...
            <>
              <Alert
                type="warning"
                showIcon
                message={`本批次已形成 ${run.auto_accepted_count} 张生产结果`}
                description="其余图纸没有形成拆板成品，不计入生产结果；请下载本批原图包线下处理。"
                action={(
                  <Space wrap>
                    <Button
                      type="primary"
