...
分批导出使用 `<a>` 触发浏览器原生下载，不把大 ZIP 整体放进 Axios Blob。选择弹窗显示
...
相邻的“导出”按钮同样使用路径级 HttpOnly 能力和原生浏览器流式下载，但只处理当前拆板
