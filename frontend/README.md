...
Playwright 默认通过 Nginx `8080`。真实 Excel Final 摘要闭环需要 `PLAYWRIGHT_EXCEL_SAMPLE_PATH` 指向业务有效样本；route fixture 只能证明 UI/API contract，不能证明 MySQL/Celery/MinIO 实际链路。生产流程页面当前是人工编排，不会自动创建 Job 或挂接产物。详见[验证文档](../docs/workflow-verification.md)。
