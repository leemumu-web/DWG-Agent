...
核心服务为 `nginx`、`backend-api`、`mysql`、`minio` 与 `worker-report`。`workers` profile 增加 `worker-maintenance`、`worker-dxf`、`worker-dxf2dwg`、`worker-dxf2excel`、`worker-dxf-classification`、`worker-dxf-split`、`worker-excel-final`、`worker-excel-stage2`、`worker-remnant-convert`、`worker-remnant-parse`。其中 `worker-excel-stage2` 独占 `excel_stage2` 队列，默认并发 1，工作目录固定落在 `app_var`。所有 worker 只出站访问 MySQL 与 MinIO，不新增 HTTP 监听端口。
