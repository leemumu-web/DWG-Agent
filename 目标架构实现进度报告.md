...
> 2026-07-19 增量更新：新增 `linux_production` 九阶段框架，公开接通 `/files` 引用、DXF→Excel、Excel Final、Job attempt 同步、结果产物和 active Job 取消；React 生产流程页同步支持文件管理与真实执行。图纸拆板、CAM 工作包、Windows Node Agent/SinoCAM 和结果接纳仍为明确的 placeholder/external 接口。本报告其余章节保留 2026-07-18 审计快照，涉及“目标业务主链尚未接通”的表述应结合本增量理解：Linux 编排框架与 Excel 两阶段已接通，Windows/CAM 核心链仍未实现。
