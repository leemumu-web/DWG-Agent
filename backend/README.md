...
运行规则：`main.py` 只保留稳定 ASGI facade，`bootstrap/` 装配应用、模型与任务，`platform/` 提供数据库、HTTP、配置、消息、安全和存储技术接口，业务逐步归入 `modules/`。Identity/projects/files/jobs 已完成归域，跨领域代码只导入各自 `interface.py`。Job worker 的领取、进度、终态和恢复写入必须匹配 status + attempt；当前 SSE 只持久化最新状态，不具备事件编号补发。Local/MinIO 保存字节，MySQL `files` 保存权限元数据和 SHA-256，`file_transfers` 保存补偿账本。Agent/CAD task 是占位，相关 flag 保持 false。
