...
def list_pl_split_candidate_inputs(
    db: Session,
    workflow_id: int,
) -> list[DxfSplitCandidateInput]:
    """Return only classifier-confirmed PL DXFs for the independent PL Stage."""
    run = latest_classification_run(db, workflow_id)
    if run is None:
        return []
    inputs: list[DxfSplitCandidateInput] = []
    for item in run.items:
        ...
        if (
            ...
            raise ClassificationError("PL 拆板候选分类输出文件不可用。")
        inputs.append(
            DxfSplitCandidateInput(
                classification_item_id=item.id,
                drawing_id=item.drawing_id,
