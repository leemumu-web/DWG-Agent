...
def validate_input_folder_manifest(
    upload_names: list[str],
    relative_paths: list[str],
) -> str:
    """Validate one browser-selected production folder before storing any object."""
    ...
    unsupported = sorted(
        {extension for extension in extensions if extension not in {".dwg", *EXCEL_FILE_EXTENSIONS}}
    )
    if unsupported:
        raise AppHTTPException(
            422,
            "INPUT_FOLDER_FILE_TYPE_NOT_ALLOWED",
            "The selected folder may contain only DWG files and one Excel file.",
    ...
    if sum(extension in EXCEL_FILE_EXTENSIONS for extension in extensions) != 1:
        raise AppHTTPException(
            422,
            "INPUT_FOLDER_EXCEL_COUNT_INVALID",
            "The selected folder must contain exactly one Excel file.",
        )
