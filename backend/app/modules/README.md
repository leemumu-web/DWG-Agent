...
Identity, projects and files are fully grouped vertical slices. Files separates registry rules and compensation from the Local/MinIO byte adapter seam. Operations already exposes the stable audit write interface. During the incremental migration, code not yet grouped remains in the legacy `models`, `schemas`, `services`, `api` and `workers` packages and is moved only with its focused regression suite.
