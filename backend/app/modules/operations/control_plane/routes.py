...
from app.modules.automation.contracts.interface import windows_node_contract
...
def list_events(
    ...
    _user=Depends(reader),
):
    rows, total = paginate_scalars(
        db,
        select(ControlPlaneEvent).order_by(ControlPlaneEvent.created_at.desc()),
        page_no=page_no,
        page_size=page_size,
    )
...
def list_messages(
    ...
    rows, total = paginate_scalars(
        db,
        select(PlatformMessage).order_by(PlatformMessage.created_at.desc()),
        page_no=page_no,
        page_size=page_size,
    )
    return page(
        [_message(row) for row in rows], page_no, page_size, total, request.state.request_id
    )
...
def queue_stale_job_reconciliation(
    request: Request, db: DbSession, _user=Depends(require_roles(ROLE_ADMIN))
):
    ...
    db.commit()
    try:
        from app.modules.operations.control_plane.tasks import reconcile_stale_jobs_task
