...
- 内部 `整理表` 的中文类型在入库时显式转换为稳定英文枚举，例如 `板材 -> plate`、`BBH腹 -> bbh_web`、`BOX腹 -> box_web`、`圆钢 -> round_bar`、`圆管/钢管 -> steel_pipe`；未知类型拒绝入库。下载工作簿的类型列只显示八种 BBH/BH/BOX/BT 子板类型，不影响内部完整类别入库。
