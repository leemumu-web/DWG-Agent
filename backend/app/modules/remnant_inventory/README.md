...
本目录是全厂共享余料、材质目录和余料导入账本的唯一业务 owner。`models.py` 定义标准材质、材质别名、导入批次、导入项、正式余料和零件编号六张表；`materials.py` 负责牌号、别名和同系列解析；`schemas.py` 定义输入输出契约；`access.py` 定义工人使用和管理员维护的角色判定；`__init__.py` 只声明领域包，不承担隐式装配。
