...
本目录是全厂共享余料、材质目录和余料导入账本的唯一业务 owner。`models.py` 定义六张表；`materials.py` 管理材质；`imports.py` 登记批次；`execution.py` 执行 attempt-fenced 转换解析；`stage_adapter.py` 调用独立解析 Stage；`tasks.py` 提供两个专用 Celery 入口；`schemas.py` 定义契约；`access.py` 定义角色判定；`__init__.py` 不承担隐式装配。
