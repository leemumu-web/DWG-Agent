...
@router.get("")
def list_jobs(request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
...
def create_job_api(payload: JobCreate, background_tasks: BackgroundTasks, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
...
@router.get("/{job_id}")
def get_job(job_id: int, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
...
def cancel_job(job_id: int, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
    job = db.get(Job, job_id)
    if not job:
        raise not_found("Job")
...
def retry_job(job_id: int, background_tasks: BackgroundTasks, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
    job = db.get(Job, job_id)
    if not job:
        raise not_found("Job")
    job.status = "queued"
...
def get_job_steps(job_id: int, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
...
def get_job_logs(job_id: int, request: Request, current_user: CurrentUser):
    return ok({"job_id": job_id, "logs": [], "message": "Structured worker logs are not wired in stage 1."}, request.state.request_id)


@router.get("/{job_id}/events")
def get_job_events(job_id: int, request: Request, current_user: CurrentUser):
...
def get_job_results(job_id: int, request: Request, current_user: CurrentUser, db: Session = Depends(get_db)):
