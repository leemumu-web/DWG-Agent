...
def test_matching_bolt_mark_leader_proves_hidden_bolt_representation() -> None:
    hidden = ProjectedCircularOpening(
        center=(100.0, 50.0),
        radius_mm=10.0,
        source_ids=(
    ...
    mark_entities = (
        ...
        SourceEntityIR(
            source_id="insert:mark/underline",
            group_id="insert:mark",
            handle="underline",
            kind="LINE",
            layer="BoltMark",
    ...
    source = SourceDocumentIR(
        path=Path("synthetic-hidden-bolt-mark.dxf"),
        dxf_version="AC1021",
        units=4,
        declared_codepage="ANSI_936",
        detected_encoding="utf-8",
        ...
        groups=(
            ObjectGroupIR(
                ...
                rotation=0.0,
                scale=(1.0, 1.0, 1.0),
                source_ids=tuple(entity.source_id for entity in mark_entities),
                layers=("BoltMark",),
            ),
        ),
        entities=mark_entities,
    )
    frame = ViewFrame(
        origin=(0.0, 0.0),
        longitudinal_axis=(1.0, 0.0),
        ...
        longitudinal_max=1000.0,
        transverse_min=0.0,
        transverse_max=500.0,
    )
    assignment = ViewAssignmentCandidate(
        h_view=PartViewIR("insert:H", "H", (), frame),
        b_view=PartViewIR("insert:B", "B", (), frame),
        h_span_error=0.0,
        b_span_error=0.0,
        score=0.0,
    )

    obligation = _opening_representation_obligation(
        (hidden,),
        source=source,
    ...
    assert "insert:mark/text" in obligation.evidence[0].source_ids
    assert "insert:mark/leader" in obligation.evidence[0].source_ids
