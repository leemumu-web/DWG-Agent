...
class TestMigrationChain:
    ...
    def test_migration_file_count(self):
        assert len(list(VERSIONS_DIR.glob("*.py"))) == 34
    ...
    def test_dag_is_acyclic_and_every_revision_is_reachable_from_head(self):
        ...
        visit(EXPECTED_HEAD)
        assert len(visited) == len(chain) == 34
...
class TestSeedIdempotency:
    def test_init_db_twice_creates_no_duplicates(self, db):
        ...
        db.expire_all()
        assert db.scalar(select(func.count()).select_from(Role)) == len(ROLE_SEEDS) == 4
        assert (
            db.scalar(select(func.count()).select_from(Permission)) == len(PERMISSION_SEEDS) == 3
        )
        admins = db.scalars(
