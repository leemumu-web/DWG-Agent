...
def _admin_headers(client: TestClient) -> dict[str, str]:
    init_db()
    login = client.post("/api/v1/auth/sessions", json={"username": "admin", "password": "SuperAdminPass1"})
...
def test_worker_activity_persists_event_and_stale_state(db):
    record_worker_activity(db, worker_name="dispatch@test", status="online", event_type="worker.online", queues=["dispatch"], concurrency=1)
    db.commit()
    client = TestClient(app)
