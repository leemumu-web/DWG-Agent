...
`test_import_batches.py` 覆盖 DXF 结构校验、混合 DWG/DXF 登记、配置上限、ZIP 拒绝和两级 SHA-256 重复检测。`test_execution.py` 覆盖单次目录 ODA 调用、解析 attempt fencing、批次计数以及专用 Celery 队列。后续测试覆盖人工校正与幂等确认、库存检索、并发预占、原图下载权限和 HTTP envelope。
