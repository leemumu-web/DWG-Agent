...
`test_import_batches.py` 覆盖 DXF 结构校验、混合 DWG/DXF 登记、配置上限、ZIP 拒绝和两级 SHA-256 重复检测。后续测试文件按实施阶段加入，覆盖异步 attempt fencing、人工校正与幂等确认、库存检索、并发预占、原图下载权限和 HTTP envelope。
