...
def test_contract_snapshot_locks_every_public_surface() -> None:
    ...
    assert len(snapshot["http_paths"]) == 194
    assert len(snapshot["http_operations"]) == 223
    assert len(snapshot["orm_tables"]) == 50
    assert len(snapshot["celery_tasks"]) == 19
    assert len(snapshot["celery_task_routes"]) == 16
    ...
    assert (
        "app.workers.tasks_pl_dxf_split.* -> dxf_split"
        in snapshot["celery_task_routes"]
    )
    assert snapshot["alembic_heads"] == ["e6b1f9a2c470"]
