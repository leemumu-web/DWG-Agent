...
if [ -f "$NGINX_PIDFILE" ] && process_exists "$NGINX_PID"; then
    nginx -p "$PROJECT_ROOT/" -c "$NGINX_CONF" -s quit 2>/dev/null && ok "Nginx 已停止" || warn "Nginx 停止失败"
else
    nginx -p "$PROJECT_ROOT/" -c "$NGINX_CONF" -s quit 2>/dev/null && ok "Nginx 已停止" || ok "Nginx 未运行"
fi
