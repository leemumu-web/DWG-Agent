...
目录迁移前后必须运行 `make architecture-check`。它比较 `runtime-contract.json` 与当前 FastAPI、ORM、Celery、React router、Compose 和 Alembic，锁定 14 个任务名及 13 条 `pattern -> queue` 路由，并验证 `module-catalog.json` 对 46 张表、195 个 HTTP operation 与 14 个任务名的唯一归属。只有有意修改外部契约并同步兼容性说明时才可执行快照脚本的 `--write`；普通重构不得用重写快照消除失败。
